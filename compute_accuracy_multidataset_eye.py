...
def calculate_metrics_for_dataset(dataset_items):
    ...
    for item in dataset_items:
        ...
        answer_type = item.get('answer_type', '').lower()  # 将answer_type转换为小写
        question_type = item.get('question_type', 'open').lower()  # 确保统一格式
        ...
        if question_type == 'open':
            ...
            if answer_type == 'diagnosis':
                # 对于'diagnosis'类型，检查gt_ans中以逗号分隔的所有标签是否都出现在pred_value中
                labels = [label.strip() for label in gt_value.split(',')]
                correct = all(label in pred_value for label in labels)
            else:
                # 对于'modality'或'laterality'类型，如果gt_ans出现在pred_value中，则认为回答正确
                correct = gt_value in pred_value or pred_value in gt_value
...
# 读取 JSON 文件
with open('modality_laterality_diagnosis_v5_test_all_ours.json', 'r') as f:
